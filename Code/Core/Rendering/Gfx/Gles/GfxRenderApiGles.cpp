...
namespace CC::Gfx
{
    ...
    // ========================
    // Render targets (stubbed for v1)
    // ========================

    RenderTargetHandle RenderApiGles::CreateRenderTarget(const RenderTargetDescription& description)
    {
        (void)description;
        CC_ASSERT(false, "CreateRenderTarget not yet implemented in GLES backend");
        return RenderTargetHandle();
    }
    ...
    GLuint RenderApiGles::GetGlShaderProgram(ShaderHandle handle) const
    {
        GLuint result = 0;
    ...
    void RenderApiGles::BindShaderProgram(ShaderHandle shader)
    {
        GLuint program = GetGlShaderProgram(shader);
        glUseProgram(program);
    }

    int RenderApiGles::GetUniformLocation(ShaderHandle shader, const char* name)
    {
        int result = -1;
        GLuint program = GetGlShaderProgram(shader);
    ...
    void RenderApiGles::SetUniformMat4(int location, const float* values)
    {
        if (location != -1)
        {
    ...
    void RenderApiGles::SetUniformVec4(int location, const float* values)
    {
        if (location != -1)
        {
    ...
    void RenderApiGles::SetUniformVec3(int location, const float* values)
    {
        if (location != -1)
        {
    ...
    void RenderApiGles::SetUniformVec2(int location, const float* values)
    {
        if (location != -1)
        {
    ...
    void RenderApiGles::SetUniformFloat(int location, float value)
    {
        if (location != -1)
        {
    ...
    void RenderApiGles::SetUniformInt(int location, int value)
    {
        if (location != -1)
        {
