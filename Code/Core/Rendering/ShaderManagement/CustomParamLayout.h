#ifndef CUSTOMPARAMLAYOUT_H
#define CUSTOMPARAMLAYOUT_H

#include <string>

namespace CC
{
    enum class CustomParamType
    {
        Float = 0,
        Int,
        Vector2,
    ...
    // declares for custom parameters that no standard update frequency
    // covers — procedural-art controls, the fade colour, the fullscreen
    // aspect ratio.
    //
    ...
    public:
        ...
        const CustomParam* FindParam(const std::string& name) const;
    ...
    private:
        ...
        static bool GetTypeInfo(const std::string& typeName, CustomParamType& outType,
                                int& outAlignmentBytes, int& outSizeBytes);
        static std::string StripLineComments(const std::string& text);

        CustomParam params[MAX_PARAMS];
        int         paramCount = 0;
