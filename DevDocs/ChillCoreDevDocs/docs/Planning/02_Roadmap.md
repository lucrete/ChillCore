...
| --- | --- | --- | --- | --- |
| 1 | Render target follow-ups | Rendering | `RenderingPlan.md` | Array-layer and cubemap-face attachments, and the backbuffer as a pool handle. The attachment work gates shadow cascades, reflection probes, and the graded LUT in row 2, so it is the cheapest way to unblock the most. |
| 2 | Post-processing follow-ups | Rendering | `RenderingPlan.md` | Per-channel grading first, then a caller-defined effect chain, then the baked LUT, procedural art through the tone curve, and photometric light units. The chain is the ceiling on the whole stack; the LUT depends on row 1. |
| 3 | Frame-rate dependent camera control | Rendering | `RenderingPlan.md` | Free-camera look and movement are paced per frame, not per second. Hours of work, user-visible, and independent of the rest of the block. |
| 4 | Dirty-state cache beyond pipelines | Rendering | `RenderingPlan.md` | Cheapest call-count win. |
| 5 | Compute shaders | Rendering | `RenderingPlan.md` | Backend surface is built. Gated on shader compilation recognising a `#shader compute` stage — that one change unblocks the rest. Ends with a compute demo state. |
...
- **Rendering (1–5) is a block, scheduled first by priority, not dependency.** No rendering item blocks the tracker, Android, or build work.
- **Within rendering, only one dependency binds: row 1 before the baked LUT in row 2.** The rest of the block can be reordered on preference.
