...
| 7 | Particle system | Rendering | `RenderingPlan.md` | The first thing in the engine to produce geometry per frame rather than load it once. Blocked on nothing; needs per-instance vertex attributes, which the vertex layout cannot describe, and a blend mode a material can choose, which none can. Carries one open decision — blending and sorting — to settle before building. |
