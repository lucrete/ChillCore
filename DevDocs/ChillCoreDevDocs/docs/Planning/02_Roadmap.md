...
| 7 | Particle system | Rendering | `RenderingPlan.md` | The first thing in the engine to produce geometry per frame rather than load it once. Blocked on nothing; needs per-instance vertex attributes, which the vertex layout cannot describe, and additive blending, which no material can ask for. |
